...
export default function CuttingStation() {
  ...
  const fetchWorkOrders = useCallback(async () => {
    ...
    setError(null)
    try {
      const response = await fetch('/api/work-orders/station/cutting')
  ...
  const fetchWorkOrderData = useCallback(async (workOrderId: string) => {
    try {
      const response = await fetch(`/api/work-orders/${workOrderId}/station-data?station=CUTTING`)
  ...
  const handleBulkComplete = async (itemIds: string[], isCompleted: boolean) => {
    ...
    try {
      ...
      // Update local state with all items
      setWorkOrderItems(data.items || [])
    } catch (err) {
  ...
  const handlePrintCutList = () => {
    if (!selectedWorkOrder) return
    window.open(`/api/work-orders/${selectedWorkOrder.id}/cut-list/pdf`, '_blank')
  }
  ...
  const completedItems = extrusionItems.filter(i => i.isCompleted)
  const progressPercent = extrusionItems.length > 0
    ? Math.round((completedItems.length / extrusionItems.length) * 100)
    : 0
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-2xl font-bold">
                    {selectedWorkOrder
                      ? `Cutting - ${selectedWorkOrder.project.name}`
                      : 'Cutting Station'
                    }
                  </h1>
                ...
                >
                  ...
                  {stageCounts[stage] > 0 && (
                    <span className={`ml-2 px-1.5 py-0.5 text-xs rounded-full ${
                      stage === 'CUTTING' ? 'bg-gray-100 text-gray-600' : 'bg-white/20'
                    }`}>
                      {stageCounts[stage]}
                    </span>
                  )}
        ...
        {selectedWorkOrder ? (
              ...
              <div className="bg-white rounded-lg border p-4">
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-gray-600">Cuts Complete</span>
                    <span className="font-medium">{completedItems.length} / {extrusionItems.length}</span>
                  </div>
          ...
          <div>
            <div className="mb-4">
              ...
              <p className="text-sm text-gray-600">Select a work order to start cutting</p>
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {workOrders.map(wo => (
                  <button
                    ...
                    className="bg-white rounded-lg border p-4 text-left hover:border-orange-500 hover:shadow-md transition-all"
                  >
                    {/* Color stripe */}
