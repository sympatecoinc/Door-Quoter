/**
 * Pricing Calculator Output Verification Script
 *
 * Creates 6 test projects with known configurations via API,
 ...
 *
 * Run with: npx tsx scripts/verify-pricing-output.ts
 */
...
const OUTPUT_DIR = path.join(process.cwd(), 'verify-output')
...
async function authenticate(): Promise<void> {
  ...
  if (setCookie) {
    // Parse out session_token from the set-cookie header
...
async function discoverPresets(): Promise<{ presetA: Preset; presetB: Preset }> {
  console.log('\n=== Step 2: Discovering Presets ===')
  const data = await apiJSON('GET', '/api/opening-presets')
  const presets: any[] = data.presets.filter((p: any) => !p.isArchived)

  console.log(`  Found ${presets.length} active presets:`)
  for (const p of presets) {
    console.log(`    - ${p.name} (id=${p.id}, ${p.defaultRoughWidth || '?'}W x ${p.defaultRoughHeight || '?'}H)`)
  }

  if (presets.length < 2) {
    throw new Error(`Need at least 2 active presets, found ${presets.length}`)
  }

  // Use first two presets as Preset A and Preset B
  const presetA = presets[0]
  const presetB = presets[1]

  console.log(`  Preset A: "${presetA.name}" (id=${presetA.id})`)
  console.log(`  Preset B: "${presetB.name}" (id=${presetB.id})`)
...
interface TestProject {
  ...
  name: string
  dir: string
  openings: { presetId: number; presetName: string; name: string; roughWidth: number; roughHeight: number; finishedWidth: number; finishedHeight: number }[]
}

async function createTestProjects(presetA: Preset, presetB: Preset): Promise<TestProject[]> {
  console.log('\n=== Step 3: Creating Test Projects ===')

  // Default dimensions if preset doesn't have them
  const aRW = presetA.defaultRoughWidth || 96
  const aRH = presetA.defaultRoughHeight || 84
  const aFW = presetA.defaultFinishedWidth || aRW
  const aFH = presetA.defaultFinishedHeight || aRH

  const bRW = presetB.defaultRoughWidth || 96
  const bRH = presetB.defaultRoughHeight || 84
  const bFW = presetB.defaultFinishedWidth || bRW
  const bFH = presetB.defaultFinishedHeight || bRH

  const openingA = (name: string) => ({
    presetId: presetA.id,
  ...
  const aShort = presetA.name.replace(/[^a-zA-Z0-9]/g, '-').substring(0, 20)
  const bShort = presetB.name.replace(/[^a-zA-Z0-9]/g, '-').substring(0, 20)

  const projectDefs = [
    { name: `Verify-01-Single-A`, dir: '01-single-preset-a', openings: [openingA('Opening 1')] },
    { name: `Verify-02-Single-B`, dir: '02-single-preset-b', openings: [openingB('Opening 1')] },
    { name: `Verify-03-Double-A`, dir: '03-double-preset-a', openings: [openingA('Opening 1'), openingA('Opening 2')] },
    { name: `Verify-04-Double-B`, dir: '04-double-preset-b', openings: [openingB('Opening 1'), openingB('Opening 2')] },
    { name: `Verify-05-Mixed-Single`, dir: '05-mixed-single', openings: [openingA('Opening 1'), openingB('Opening 2')] },
    { name: `Verify-06-Mixed-Double`, dir: '06-mixed-double', openings: [openingA('Opening 1'), openingA('Opening 2'), openingB('Opening 3'), openingB('Opening 4')] },
  ]

  console.log(`  Preset A: "${presetA.name}" (${aRW}W x ${aRH}H)`)
  console.log(`  Preset B: "${presetB.name}" (${bRW}W x ${bRH}H)`)
  ...
  for (const def of projectDefs) {
    ...
    const project = await apiJSON('POST', '/api/projects', {
      name: def.name,
      prospectCompanyName: 'Verification Test',
      status: 'STAGING'
    })
...
async function applyPresets(projects: TestProject[]): Promise<void> {
  console.log('\n=== Step 4: Applying Presets ===')

  for (const project of projects) {
    for (const opening of project.openings) {
      console.log(`  Applying "${opening.presetName}" to ${project.name} → ${opening.name}`)
      await apiJSON('POST', `/api/opening-presets/${opening.presetId}/apply`, {
...
// Format: Part Number,Part Name,Type,Size (WxH),Pieces,Unit
...
function compareProjectPair(
  ...
  if (fs.existsSync(singlePricingPath) && fs.existsSync(doublePricingPath)) {
    ...
    // Each opening in the double project should have identical BOM items to the
    // corresponding opening in the single project. We compare per-opening to handle
    // mixed presets correctly.
    ...
    if (singleOpeningCount > 0 && doubleOpeningCount === singleOpeningCount * multiplier) {
      // Parse BOM items per opening from the raw CSV
      ...
      // (openings with the same preset will have the same part numbers in the same order)
      for (let d = 0; d < doubleOpeningBoms.length; d++) {
        const dItems = doubleOpeningBoms[d] || []
        const dSig = dItems.map(i => i.partNumber).join('|')

        // Find the single opening with the best matching signature
        let bestSIdx = 0
      ...
      // Fallback: just check that all BOM item count is multiplier×single
  ...
  if (fs.existsSync(singleBomPath) && fs.existsSync(doubleBomPath)) {
    ...
    // Build maps: part number → total pieces
    ...
    // Yield optimization may change stock lengths, so INFO not FAIL for differences
    const bomYieldNote = bomMismatchDetails.length > 0
      ? `Yield optimization: bin-packing ${multiplier}x cuts together may use fewer stock pieces than ${multiplier} × single. Details: ${bomMismatchDetails.slice(0, 5).join('; ')}`
      : ''
    results.push({
  ...
  if (fs.existsSync(singlePurchPath) && fs.existsSync(doublePurchPath)) {
    ...
    // Check pieces scale by multiplier
    ...
    // Also check for parts in double that aren't in single
    ...
    // Yield optimization may cause stock piece counts to differ (INFO, not FAIL)
    const purchYieldNote = mismatchDetails.length > 0
      ? `Yield optimization: bin-packing ${multiplier}x cuts together may use fewer stock pieces than ${multiplier} × single. Details: ${mismatchDetails.slice(0, 5).join('; ')}${mismatchDetails.length > 5 ? '...' : ''}`
      : ''
    results.push({
...
function crossCompareReports(
  ...
  if (!hasPricing || !hasBom || !hasPurch) {
    console.log(`    Skipping — missing files`)
    return
  }
  ...
  // Check 1: Opening base totals sum to subtotal base
  ...
  // Check 2: Opening marked-up totals sum to subtotal marked up
  ...
  // Check 3: Pricing-debug BOM quantities vs BOM CSV pieces (by base part number)
  const pricingQtyMap = new Map<string, number>()
  for (const item of pricing.bomItems) {
    pricingQtyMap.set(item.partNumber, (pricingQtyMap.get(item.partNumber) || 0) + item.quantity)
  }
  ...
  // Check 4: BOM CSV vs Purchasing Summary pieces
  ...
  // Check 5: Grand total = subtotal marked up + installation
...
function runComparisons(results: ComparisonResult[]): void {
  ...
  // Comparison A: Project 3 (2x A) vs 2x Project 1 (1x A)
  compareProjectPair(
    'Comparison A: 2x PresetA vs 2×(1x PresetA)',
    '01-single-preset-a',
    '03-double-preset-a',
    2,
    results
  ...
  // Comparison B: Project 4 (2x B) vs 2x Project 2 (1x B)
  compareProjectPair(
    'Comparison B: 2x PresetB vs 2×(1x PresetB)',
    '02-single-preset-b',
    '04-double-preset-b',
    2,
    results
  ...
  compareProjectPair(
    'Comparison C: 2x Mixed vs 2×(1x Mixed)',
    '05-mixed-single',
    '06-mixed-double',
    2,
    results
...
async function main() {
  console.log('╔══════════════════════════════════════════════════╗')
  console.log('║  Pricing Calculator Output Verification Script  ║')
  console.log('╚══════════════════════════════════════════════════╝')
  ...
  try {
    await authenticate()
    const { presetA, presetB } = await discoverPresets()
    projects = await createTestProjects(presetA, presetB)
    await applyPresets(projects)
