...
| `AccountRef` | `ReferenceType` | No | Specifies the account reference. Check should have bank account, CreditCard should specify credit ca... |
...
| `PaymentType` | `PaymentTypeEnum` | No | Required element. No defaults. Expense Type can be Cash, Check or CreditCard Cash based expense is n... |
| `EntityRef` | `ReferenceType` | No | Specifies the party to whom a expense is associated with. Can be Customer, Vendor, Employee (or Othe... |
| `Credit` | `boolean` | No | If Credit is Null or False, it is considered as Charge. If true, the CreditCard represents a Refund.... |
| `RemitToAddr` | `PhysicalAddress` | No | Address to which the payment should be sent.Output only. |
| `TotalAmt` | `decimal` | No | The total amount due, determined by taking the sum of all lines associated. This includes all charge... |
| `Memo` | `string` | No | Memo that will be printed in check in case of Check purchase, Memo appears on the expense report for... |
| `PrintStatus` | `PrintStatusEnum` | No | PrintStatus if to be printed or already printed information |
...
| `PurchaseEx` | `IntuitAnyType` | No | Internal use only: extension place holder for Purchase. |
| `LessCIS` | `decimal` | No | Product: All Description: QBO: Indicates the less cis amount of the transaction, specific to UK regi... |
| `IncludeInAnnualTPAR` | `boolean` | No | Product: QBO Only Description: True if the Purchase should be included in annual TPAR, specific to A... |
...
Specifies the account reference. Check should have bank account, CreditCard should specify credit card account
...
Required element. No defaults. Expense Type can be Cash, Check or CreditCard Cash based expense is not supported by QBW. Filterable: QBW
...
Specifies the party to whom a expense is associated with. Can be Customer, Vendor, Employee (or OtherName in case of QBW)
...
If Credit is Null or False, it is considered as Charge. If true, the CreditCard represents a Refund. Valid only for CreditCard transaction Filterable: QBW
...
The total amount due, determined by taking the sum of all lines associated. This includes all charges, allowances, taxes, discounts, etc... [b]QuickBooks Notes[/b]
...
Memo that will be printed in check in case of Check purchase, Memo appears on the expense report for CreditCard, Memo for CashPurchase
...
Product: QBO Only Description: True if the Purchase should be included in annual TPAR, specific to AU region.
...
const createPurchase = async (realmId, accessToken, data) => {
...
```javascript
const getPurchase = async (realmId, id, accessToken) => {
  const response = await axios.get(
    `https://quickbooks.api.intuit.com/v3/company/${realmId}/purchase/${id}`,
    {
      headers: {
        'Authorization': `Bearer ${accessToken}`,
        'Accept': 'application/json'
...
```javascript
const queryPurchases = async (realmId, accessToken) => {
