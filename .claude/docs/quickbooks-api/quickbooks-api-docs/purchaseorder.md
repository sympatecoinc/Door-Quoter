...
| `VendorRef` | `ReferenceType` | No | Product: ALL Description: Specifies the vendor reference for this transaction Filterable: QBW |
| `APAccountRef` | `ReferenceType` | No | Specifies which AP account the bill will be credited to. Many/most small businesses have a single AP... |
| `TotalAmt` | `decimal` | No | Product: ALL Description: The total amount due, determined by taking the sum of all lines associated... |
| `BillEmail` | `EmailAddress` | No | Product: QBW Description: The email address to which this bill is/was sent.   Non QB-writable. |
| `ReplyEmail` | `EmailAddress` | No | Product: QBW Description: The email address to which inquiries about the bill may be directed. (Also... |
| `Memo` | `string` | No | QBW only. Memo to be visible to Payee |
...
| `TaxCodeRef` | `ReferenceType` | No | Represents the TaxCode Reference with respect to the purchase |
| `ClassRef` | `ReferenceType` | No |  |
| `ReimbursableInfoRef` | `ReferenceType` | No | Information about the Customer and actual Job or Project the expense must be reimbursed for. |
| `SalesTermRef` | `ReferenceType` | No |  |
| `DueDate` | `date` | No | The nominal date by which the bill must be paid, not including any early-payment discount incentives... |
| `ExpectedDate` | `date` | No | The date when the delivery of the product is expected. |
| `VendorAddr` | `PhysicalAddress` | No | Address to which the payment should be sent. [b]QuickBooks Notes[/b]  Non QB-writable. |
| `ShipTo` | `ReferenceType` | No | Product: QBO Description: Ship to Entity Reference. |
| `ShipAddr` | `PhysicalAddress` | No | Address to which the vendor shipped or will ship any goods associated with the purchase. |
| `ShipMethodRef` | `ReferenceType` | No |  |
| `FOB` | `string` | No | "Free On Board", specifies the terms between buyer and seller regarding transportation costs; does n... |
| `POEmail` | `EmailAddress` | No | The email address to which this purchase order is/was sent. Length Restriction: QBO: 15 QBW: 1024 |
| `TemplateRef` | `ReferenceType` | No |  |
| `PrintStatus` | `PrintStatusEnum` | No |  |
| `EmailStatus` | `EmailStatusEnum` | No |  |
| `ManuallyClosed` | `boolean` | No | The entire transaction, or individual items are manually closed, i.e. they may not be received. |
| `POStatus` | `PurchaseOrderStatusEnum` | No |  |
| `PurchaseOrderEx` | `IntuitAnyType` | No | Internal use only: extension place holder for PurchaseOrder |
| `DropShipToEntity` | `ReferenceType` | No | DropShip to Entity Reference |
| `InventorySiteRef` | `ReferenceType` | No |  |
...
Specifies which AP account the bill will be credited to. Many/most small businesses have a single AP account, so the account is implied. When specified, the account must be a Liability account, and further, the sub-type must be of type "Payables" [b]QuickBooks Notes[/b]
...
Product: ALL Description: The total amount due, determined by taking the sum of all lines associated. This includes all charges, allowances, taxes, discounts, etc... [b]QuickBooks Notes[/b]
 Non QB-writable. Output only field in case of QBO Filterable: QBW Sortable: QBW
...
Product: QBW Description: The email address to which this bill is/was sent. 
 Non QB-writable.
...
Product: QBW Description: The email address to which inquiries about the bill may be directed. (Also appropriate for paypal payments). 
 Non QB-writable.
...
The nominal date by which the bill must be paid, not including any early-payment discount incentives, or late payment penalties.
...
Address to which the payment should be sent. [b]QuickBooks Notes[/b]
 Non QB-writable.
...
Product: QBO Description: Ship to Entity Reference.
...
"Free On Board", specifies the terms between buyer and seller regarding transportation costs; does not have any bookkeeping implications. Length Restriction: QBO: 15 QBW: 1024
...
The email address to which this purchase order is/was sent. Length Restriction: QBO: 15 QBW: 1024
...
The entire transaction, or individual items are manually closed, i.e. they may not be received.
...
const createPurchaseorder = async (realmId, accessToken, data) => {
...
```javascript
const getPurchaseorder = async (realmId, id, accessToken) => {
  const response = await axios.get(
    `https://quickbooks.api.intuit.com/v3/company/${realmId}/purchaseorder/${id}`,
    {
      headers: {
        'Authorization': `Bearer ${accessToken}`,
        'Accept': 'application/json'
...
```javascript
const queryPurchaseorders = async (realmId, accessToken) => {
  const query = "SELECT * FROM Purchaseorder MAXRESULTS 100";
