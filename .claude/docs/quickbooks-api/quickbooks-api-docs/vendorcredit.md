...
| `VendorCreditEx` | `IntuitAnyType` | No | Internal use only: extension place holder for Bill extensible element to qualify account. |
| `Balance` | `decimal` | No | Product: ALL Description: The unpaid amount of the bill. When paid-in-full, balance will be zero. [b... |
| `IncludeInAnnualTPAR` | `boolean` | No | Product: QBO Only Description: True if the VendorCredit should be included in annual TPAR, specific ... |
...
Internal use only: extension place holder for Bill extensible element to qualify account.
...
Product: QBO Only Description: True if the VendorCredit should be included in annual TPAR, specific to AU region.
...
const createVendorcredit = async (realmId, accessToken, data) => {
...
```javascript
const getVendorcredit = async (realmId, id, accessToken) => {
  const response = await axios.get(
    `https://quickbooks.api.intuit.com/v3/company/${realmId}/vendorcredit/${id}`,
    {
      headers: {
        'Authorization': `Bearer ${accessToken}`,
        'Accept': 'application/json'
...
```javascript
const queryVendorcredits = async (realmId, accessToken) => {
  const query = "SELECT * FROM Vendorcredit MAXRESULTS 100";
