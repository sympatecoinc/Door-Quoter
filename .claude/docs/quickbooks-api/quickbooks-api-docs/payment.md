...
| `CustomerRef` | `ReferenceType` | No | Product: ALL Description: Represents Customer (or Job)Reference Filterable: QBW |
| `RemitToRef` | `ReferenceType` | No | Identifies the party or location that the payment is to be remitted to or sent to. [b]QuickBooks Not... |
| `ARAccountRef` | `ReferenceType` | No | ARAccountReferenceGroup Identifies the AR Account to be used for this Payment. [b]QuickBooks Notes[/... |
| `DepositToAccountRef` | `ReferenceType` | No | Optional asset account specification to designate the account the payment money needs to be deposite... |
| `PaymentMethodRef` | `ReferenceType` | No | Product: ALL Description: Reference to the PaymentMethod. |
| `PaymentRefNum` | `string` | No | Product: ALL Description: The reference number for the payment received (I.e. Check # for a check, e... |
| `PaymentType` | `PaymentTypeEnum` | No | Product: ALL Description: Valid values are Cash, Check, CreditCard, or Other. No defaults. Cash base... |
| `TotalAmt` | `decimal` | No | Product: ALL Description: Indicates the total amount of the entity associated. This includes the tot... |
| `UnappliedAmt` | `decimal` | No | Indicates the amount that has not been applied to pay amounts owed for sales transactions. [b]QuickB... |
| `ProcessPayment` | `boolean` | No | Indicates that the payment should be processed by merchant account service. Valid for QBO companies ... |
| `PaymentExtendedType` | `PaymentExtendedTypeEnum` | No | Product: ALL Description: Current only valid value is Prepayment. |
| `PaymentEx` | `IntuitAnyType` | No | Internal use only: extension place holder for Payment |
| `CheckPayment` | `CheckPayment` | No |  |
| `CreditCardPayment` | `CreditCardPayment` | No |  |
...
Product: ALL Description: Represents Customer (or Job)Reference Filterable: QBW
...
Identifies the party or location that the payment is to be remitted to or sent to. [b]QuickBooks Notes[/b]
 Non QB-writable.
...
ARAccountReferenceGroup Identifies the AR Account to be used for this Payment. [b]QuickBooks Notes[/b]
...
Optional asset account specification to designate the account the payment money needs to be deposited to. [b]QuickBooks Notes[/b]
...
Product: ALL Description: Reference to the PaymentMethod.
...
Product: ALL Description: The reference number for the payment received (I.e. Check # for a check, envelope # for a cash donation, CreditCardTransactionID for a credit card payment)
...
Product: ALL Description: Valid values are Cash, Check, CreditCard, or Other. No defaults. Cash based expense is not supported by QuickBooks Windows. Not applicable to Estimate and SalesOrder.
...
Product: ALL Description: Indicates the total amount of the entity associated. This includes the total of all the payments from the Payment Details. [b]QuickBooks Notes[/b]
...
Indicates the amount that has not been applied to pay amounts owed for sales transactions. [b]QuickBooks Notes[/b]
 Non QB-writable.
...
Indicates that the payment should be processed by merchant account service. Valid for QBO companies with credit card processing. QBO only field.

### PaymentExtendedType

**Type:** `PaymentExtendedTypeEnum`

**Required:** No

Product: ALL Description: Current only valid value is Prepayment.
...
### Create a Payment (JavaScript/Node.js)

```javascript
const axios = require('axios');

const createPayment = async (realmId, accessToken) => {
  const payment = {
    "TotalAmt": 100.00,
    "CustomerRef": {
      "value": "1"
