...
GET /v3/company/{realmId}/refundreceipt/{id}
```
...
| `RefundReceiptEx` | `IntuitAnyType` | No | Internal use only: extension place holder for Refund |
...
const createRefundreceipt = async (realmId, accessToken, data) => {
...
```javascript
const getRefundreceipt = async (realmId, id, accessToken) => {
  const response = await axios.get(
    `https://quickbooks.api.intuit.com/v3/company/${realmId}/refundreceipt/${id}`,
    {
      headers: {
        'Authorization': `Bearer ${accessToken}`,
        'Accept': 'application/json'
...
```javascript
const queryRefundreceipts = async (realmId, accessToken) => {
  const query = "SELECT * FROM Refundreceipt MAXRESULTS 100";
