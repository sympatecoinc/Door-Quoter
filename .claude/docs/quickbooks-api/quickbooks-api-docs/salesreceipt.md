...
| `SalesReceiptEx` | `IntuitAnyType` | No | Extension entity for SalesReceipt |
...
const createSalesreceipt = async (realmId, accessToken, data) => {
...
```javascript
const getSalesreceipt = async (realmId, id, accessToken) => {
  const response = await axios.get(
    `https://quickbooks.api.intuit.com/v3/company/${realmId}/salesreceipt/${id}`,
    {
      headers: {
        'Authorization': `Bearer ${accessToken}`,
        'Accept': 'application/json'
...
```javascript
const querySalesreceipts = async (realmId, accessToken) => {
  const query = "SELECT * FROM Salesreceipt MAXRESULTS 100";
