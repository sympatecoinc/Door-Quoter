...
| `PayerRef` | `ReferenceType` | No | Product: QBW Description: Payer information |
| `SalesTermRef` | `ReferenceType` | No | SalesTerm Reference for the bill |
| `DueDate` | `date` | No | The nominal date by which the bill must be paid, not including any early-payment discount incentives... |
| `RemitToAddr` | `PhysicalAddress` | No | Address to which the payment should be sent. [b]QuickBooks Notes[/b]  Non QB-writable. |
...
| `Balance` | `decimal` | No | Product: ALL Description: The unpaid amount of the bill. When paid-in-full, balance will be zero. [b... |
| `HomeBalance` | `decimal` | No | Product: ALL Description: The unpaid amount of the bill in home currency. Available only for compani... |
| `BillEx` | `IntuitAnyType` | No | Internal use only: extension place holder for Bill. |
| `LessCIS` | `decimal` | No | Product: All Description: QBO: Indicates the less cis amount of the transaction, specific to UK regi... |
| `IncludeInAnnualTPAR` | `boolean` | No | Product: QBO Only Description: True if the Bill should be included in annual TPAR, specific to AU re... |
...
Product: QBW Description: Payer information
...
Product: ALL Description: The unpaid amount of the bill. When paid-in-full, balance will be zero. [b]QuickBooks Notes[/b]
 Non QB-writable. Filterable: QBW Sortable: QBW
...
Product: ALL Description: The unpaid amount of the bill in home currency. Available only for companies where multicurrency is enabled. When paid-in-full, home balance will be zero. [b]QuickBooks Notes[/b]
 Non QB-writable. Filterable: QBW Sortable: QBW
...
Product: QBO Only Description: True if the Bill should be included in annual TPAR, specific to AU region.
...
### Create a Bill (JavaScript/Node.js)

```javascript
const axios = require('axios');

const createBill = async (realmId, accessToken) => {
  const bill = {
    "VendorRef": {
      "value": "1"
    },
