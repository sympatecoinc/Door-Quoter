...
| `ExpirationDate` | `date` | No | Date by which estimate must be accepted before invalidation. QBO only field. |
| `AcceptedBy` | `string` | No | Name of customer who accepted the estimate. QBO only field. |
| `AcceptedDate` | `date` | No | Date estimate was accepted. QBO only field. |
| `EstimateEx` | `IntuitAnyType` | No | Extension entity for Estimate |
...
Date by which estimate must be accepted before invalidation. QBO only field.
...
Name of customer who accepted the estimate. QBO only field.
...
Date estimate was accepted. QBO only field.
...
### Create an Estimate (JavaScript/Node.js)

```javascript
const axios = require('axios');

const createEstimate = async (realmId, accessToken) => {
  const estimate = {
    "CustomerRef": {
      "value": "1"
    },
