...
| `DepositToAccountRef` | `ReferenceType` | No | DepositToAccountReferenceGroup Identifies the Asset Account (bank account) to be used for this Depos... |
| `CashBack` | `CashBackInfo` | No |  |
...
| `TotalAmt` | `decimal` | No | Total amount of Deposit. [b]QuickBooks Notes[/b]  Non QB-writable. |
...
| `DepositEx` | `IntuitAnyType` | No | Internal use only: extension place holder for Deposit |
...
DepositToAccountReferenceGroup Identifies the Asset Account (bank account) to be used for this Deposit. [b]QuickBooks Notes[/b]
...
Total amount of Deposit. [b]QuickBooks Notes[/b]
 Non QB-writable.
...
const createDeposit = async (realmId, accessToken, data) => {
  const response = await axios.post(
    `https://quickbooks.api.intuit.com/v3/company/${realmId}/deposit`,
    data,
    {
      headers: {
...
```javascript
const getDeposit = async (realmId, id, accessToken) => {
  const response = await axios.get(
    `https://quickbooks.api.intuit.com/v3/company/${realmId}/deposit/${id}`,
    {
      headers: {
        'Authorization': `Bearer ${accessToken}`,
        'Accept': 'application/json'
...
```javascript
const queryDeposits = async (realmId, accessToken) => {
  const query = "SELECT * FROM Deposit MAXRESULTS 100";
